...
binarized_image <- load.image("output/processed_image.jpg")
...
count_patch <- function(xorig, yorig, patch_size, blur_sigma, thr_centers, im){
  ...
  small_patch <- imsub(im, x %in% xorig:(xorig + patch_size), y %in% yorig:(yorig + patch_size))
  ...
  blurry_patch <- isoblur(1 - small_patch, sigma = blur_sigma)
...
xt <- seq(from = aoi_dims$xmin, aoi_dims$xmax, by = 50) 
yt <- seq(from = aoi_dims$ymin, aoi_dims$ymax, by = 50) 
...
all_centers <- all_centers %>% dplyr::filter(n > 3)
...
small_patch <- imsub(binarized_image, x %in% (xorig_check):(xorig_check + patch_size_check), 
...
my_image <- load.image("output/grayscale_image.jpg")
plot(my_image) 
